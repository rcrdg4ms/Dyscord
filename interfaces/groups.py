...
class Boxlg(BoxLayout):
    ...
    def change_group(self, groupid):
        if localdb.get()['group'] == groupid:
                return
        for children in self.children:
            if type(children) != Widget:
                if children.groupid == localdb.get()['group']:
                    Animation(
                        pingsize = 1,
...
class Group(Widget):
    def __init__(self, groupid = None, **kwargs):
        ...
        if self.groupid == localdb.get('group'):
            self.pingsize = 42
    ...
    def enter(self):
        Animation(
            pingsize = 20 if self.groupid != localdb.get()['group'] else self.pingsize,
    ...
    def leave(self):
        Animation(
            pingsize = 1 if self.groupid != localdb.get()['group'] else self.pingsize,
